...
- Runtime milestone: a Windows release build of this exact artifact clears
  ...
  escaping, immutable dictionary values, and guest `statfs`. It reaches the
  game's Origin/ad-data setup, executes its offline download and tracking
  failure handling, initializes its tracking database, and requests locale
  information, but does not yet show a stable screen or gameplay loop.
- Current implementation: reusable Foundation KVC/dictionary, data, assertion,
  ...
  are implemented.
- Current frontier: `CTTelephonyNetworkInfo` is missing after tracking-database
  startup. Model the smallest device-information API needed by the caller, then
