...
pub fn app_picker(options: Options) -> Result<(PathBuf, Vec<String>), String> {
    ...
    let apps: Result<Vec<AppInfo>, String> = if !apps_dir.is_dir() {
        Err(format!("The {} directory couldn't be found. Check you're running tapHLE from the right directory.", apps_dir.display()))
    } else {
...
pub const DYLIB: crate::dyld::HostDylib = crate::dyld::HostDylib {
    ...
    path: "/.tapHLE/AppPickerHelpers.dylib",
    aliases: &[],
...
@implementation _tapHLE_AppPickerDelegate: NSObject
...
- (())visitWebsite {
    ...
    let url = ns_string::get_static_str(env, "https://github.com/ephun/tapHLE");
...
fn app_picker_inner(
    ...
    let delegate = env
        .objc
        .get_known_class("_tapHLE_AppPickerDelegate", &mut env.mem);
    ...
    make_button_row(
        ...
        &[
            ...
            ("tapHLE repository", "visitWebsite"),
        ],
        None,
    );
    ...
    // If an app is picked, this loop returns. If the user quits tapHLE, the
    // process exits.
    let app_path = loop {
        ...
        if icon_tapped != nil {
            match icon_grid_stuff.as_ref().unwrap().icon_map.get(&icon_tapped) {
                Some(&TappedIcon::App(app_idx)) => {
                    ...
                    // Ensure tapHLE is responsive from the OS perspective,
