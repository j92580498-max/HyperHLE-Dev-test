...
//!
//! See also [crate::paths], which has paths for host files used by tapHLE.
...
enum FileLocation {
    ...
    /// Name of a resource file bundled with tapHLE. Read only.
...
impl GuestPath {
    ...
    ///
    /// Warning! This function should only be used for internal tapHLE
    /// purposes.
...
fn handle_open_err<T, E: std::fmt::Display, P: std::fmt::Debug>(
    ...
    match open_result {
        Ok(ok) => ok,
        Err(e) => panic!("Unexpected I/O failure when trying to access real path {host_path:?}: {e}. This might indicate that files needed by tapHLE are missing, or were moved while it was running."),
    }
}
...
impl Fs {
    ...
    pub fn new(
        ...
        // Some Free Software libraries are bundled with tapHLE.
